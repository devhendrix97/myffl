...
function formatProviderStats(stats: Record<string, unknown>): string {
  return Object.entries(stats).filter(([, value]) => Number(value) !== 0).map(([key, value]) => `${key.split(":")[1] ?? key} ${String(value)}`).join(" - ") || "No recorded stats";
}
...
function PlayersView({ league, accessToken, onNavigate }: { league: LeagueDetail; accessToken: string; onNavigate: (tab: LeagueTab) => void }) {
  ...
  if (profile) return <div className="player-directory player-profile-page">
    ...
    <section className="player-profile detail-mode">
      ...
      {profile.seasonOutlook && <SeasonOutlook outlook={profile.seasonOutlook}/>}
      <div className="profile-stats-grid">
        <section className="profile-games"><h3>Yearly stats</h3>{profile.yearlyStats.length ? profile.yearlyStats.map((season) => <div key={season.seasonYear}><span>{season.seasonYear}<small>{season.games} games</small></span><code>{formatProviderStats(season.stats)}</code></div>) : <p className="muted-empty">Yearly totals will appear after myFFL has ingested season history for this player.</p>}</section>
        <section className="profile-games"><h3>Recent games</h3>{profile.recentGames.length ? profile.recentGames.map((game) => <div key={game.eventId}><span>{game.eventId}</span><code>{formatProviderStats(game.stats)}</code></div>) : <p className="muted-empty">No recent production game statistics.</p>}</section>
      </div>
    </section>
  </div>;
...
function TeamView({ league, accessToken }: { league: LeagueDetail; accessToken: string }) {
  ...
  return <div className="team-room">
    ...
    {profile && <div className="profile-scrim" role="presentation" onMouseDown={() => setProfile(null)}><section className="player-profile" role="dialog" aria-modal="true" aria-labelledby="player-profile-title" onMouseDown={(event) => event.stopPropagation()}><button className="icon-button profile-close" aria-label="Close player profile" onClick={() => setProfile(null)}><X size={18}/></button><div className="profile-title"><PlayerAvatar src={profile.headshotUrl} name={profile.displayName} size="lg"/><div><p className="eyebrow">{profile.position} - {profile.nflTeam ?? "Free agent"}</p><h2 id="player-profile-title">{profile.displayName}</h2><p>{profile.rosteredByTeamName ? `Rostered by ${profile.rosteredByTeamName}` : "Available player"}</p></div></div>{profile.injuryStatus && <span className="profile-injury">{profile.injuryStatus}</span>}<button className="outline-button compact" onClick={() => void toggleWatch()}>{profile.watched ? "Remove watch" : "Add to watchlist"}</button>{profile.seasonOutlook && <SeasonOutlook outlook={profile.seasonOutlook} compact/>}<div className="profile-games"><h3>Recent games</h3>{profile.recentGames.length ? profile.recentGames.map((game) => <div key={game.eventId}><span>{game.eventId}</span><code>{formatProviderStats(game.stats)}</code></div>) : <p className="muted-empty">No recent production game statistics.</p>}</div></section></div>}
  </div>;
}

function SeasonOutlook({ outlook, compact = false }: { outlook: string; compact?: boolean }) {
  return <section className={compact ? "profile-outlook compact" : "profile-outlook"}>
    <header><h3>Season outlook</h3><span className="espn-credit"><small>Powered by</small><b>ESPN</b></span></header>
    <p>{outlook}</p>
  </section>;
}
...
function FantasyProsAttribution({ updatedAt }: { updatedAt?: string }) {
